...
const ManageDashView = (props) => {
  ...
  async function fetchData() {
    ...
    const response = await Axios.get(`https://api.blockcypher.com/v1/dash/main/addrs/${selectedAddress.address}`);
  ...
  React.useEffect(() => {
    ...
    switch (selectedAddress.addressType) {
      case KEYPAIR_TYPES.BITCOIN:
        foundFee = shared.find((i) => i.id === 'dash-blockcypher');
        break;
      default:
        break;
    }
  ...
  const handleClickBlockExplorer = (e) => {
    e.preventDefault();
    shell.openExternal(`https://live.blockcypher.com/dash/address/${selectedAddress.address}/`);
  };
      ...
      <DialogContent style={{ backgroundColor: bgColor }}>
          ...
          <AccordionDetails>
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6} style={{ textAlign: 'center' }}>
                ...
                <QRCode value={`dash:${selectedAddress.address}`} />
              </Grid>
              <Grid item xs={12} sm={6}>
                <Typography gutterBottom>
                  Balance: {readableBalance || 0} DASH
                </Typography>
                <Typography color="textSecondary">
                  Total Received: {addressData.total_sent || 0} DASH
                </Typography>
                <Typography color="textSecondary">
                  Total Sent: {addressData.total_sent || 0} DASH
                </Typography>
                <Typography color="textSecondary">
                  Unconfirmed Balance: {addressData.unconfirmed_balance || 0} DASH
                </Typography>
                <Typography color="textSecondary">
                  Final Balance: {addressData.final_balance || 0} DASH
                </Typography>
        ...
        <Accordion>
          <AccordionSummary
            ...
            id="panel3a-header"
          >
            <Typography>Withdraw DASH</Typography>
          </AccordionSummary>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                  <Typography component="p" variant="body2">
                    Provide the amount of DASH you wish to send. A transaction fee (calculated by the DASHcoin
                ...
                {showInsufficientValue && (
                  <Grid item xs={12}>
                    <Alert severity="error">
                      ...
                      Please make sure your address has enough DASH to make a transfer,
