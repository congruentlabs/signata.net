...
const ManageLtcView = (props) => {
  ...
  async function fetchData() {
    ...
    const response = await Axios.get(`https://api.blockcypher.com/v1/ltc/main/addrs/${selectedAddress.address}`);
  ...
  React.useEffect(() => {
    ...
    switch (selectedAddress.addressType) {
      case KEYPAIR_TYPES.BITCOIN:
        foundFee = shared.find((i) => i.id === 'litecoin-blockcypher');
        break;
      default:
        break;
    }
  ...
  const handleClickBlockExplorer = (e) => {
    e.preventDefault();
    shell.openExternal(`https://live.blockcypher.com/ltc/address/${selectedAddress.address}/`);
  };
      ...
      <DialogContent style={{ backgroundColor: bgColor }}>
          ...
          <AccordionDetails>
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6} style={{ textAlign: 'center' }}>
                ...
                <QRCode value={`litecoin:${selectedAddress.address}`} />
              </Grid>
              <Grid item xs={12} sm={6}>
                <Typography gutterBottom>
                  Balance: {readableBalance || 0} LTC
                </Typography>
                <Typography color="textSecondary">
                  Total Received: {addressData.total_sent || 0} LTC
                </Typography>
                <Typography color="textSecondary">
                  Total Sent: {addressData.total_sent || 0} LTC
                </Typography>
                <Typography color="textSecondary">
                  Unconfirmed Balance: {addressData.unconfirmed_balance || 0} LTC
                </Typography>
                <Typography color="textSecondary">
                  Final Balance: {addressData.final_balance || 0} LTC
                </Typography>
        ...
        <Accordion>
          <AccordionSummary
            ...
            id="panel3a-header"
          >
            <Typography>Withdraw LTC</Typography>
          </AccordionSummary>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                  <Typography component="p" variant="body2">
                    Provide the amount of LTC you wish to send. A transaction fee (calculated by the bitcoin
